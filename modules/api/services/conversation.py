...
def end_conversation_if_needed(conversation: Conversation, **kwargs):
    ...
    if conversation_timeout(conversation, **kwargs):
        return toggle_active_conversations(conversation, **kwargs)
    return False
...
def end_conversation(conversation: Conversation, db):
    conversation.finished_at = datetime.now()
    conversation.active = False
    db.commit()
    return True
    
def toggle_active_conversations(conversation: Conversation, db):
    next_conversation = set_next_conversation_as_active(conversation, db)
    end_conversation(conversation, db)
    if next_conversation:
        db.refresh(next_conversation)
